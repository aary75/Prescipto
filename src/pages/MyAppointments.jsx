...
const MyAppointments = () => {
  ...
  const { backenedUrl, token, getDoctorsData } = useContext(AppContext)
  ...
  const getUserAppointments = async () => {
     try {
      ...
      const {data} = await axios.get(backenedUrl+'/api/user/appointments',{headers:{token}})
  ...
  const cancelAppointment = async (appointmentId) => {
    ...
    try {
      ...
      const {data} = await axios.post(backenedUrl + '/api/user/cancel-appointment', {appointmentId},{headers:{token}})
      if (data.success) {
    ...
    const options = {
      ...
      handler: async (response) => {
        ...
        try {
          ...
          const {data} = await axios.post(backenedUrl+'/api/user/verifyRazorpay',response,{headers:{token}})
          if (data.success) {
            getUserAppointments()
  ...
  const appointmentRazorpay = async (appointmentId) => {
    ...
    try {
        ...
        const {data} = await axios.post(backenedUrl+'/api/user/payment-razorpay',{appointmentId},{headers:{token}})
